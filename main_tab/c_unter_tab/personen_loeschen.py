...
def load_person_buttons(ui):
    ui.nachrichten.info("Lade Personen zum löschen...")
...
def loeschen_button_gedrückt(ui):
    ...
    try:
        ...
        if anser["success"] == True:
            ui.nachrichten.info(f"Person gelöscht: {name}")
        if anser["success"] == False:
            ui.nachrichten.error(f"Fehler beim Löschen der Person {name}: {anser['error']}")
    except Exception as e:
        ui.nachrichten.error(f"Fehler beim Löschen der Person {name}: {e}")
